#!/usr/bin/env python3
"""Fail-closed T44-T52 connected-region preparation/activation validator."""
from __future__ import annotations
import argparse,fnmatch,functools,json,pathlib,subprocess
ROOT=pathlib.Path(__file__).resolve().parents[3]; DOCS=ROOT/"Docs"/"Production"; TASKS=[f"T{i:02d}" for i in range(44,53)]
...
def prep(tid):
    ...
    c=load(d/"ACTIVATION_CHECKLIST.json"); p=load(d/"PREBUILD_CONTRACT.json"); defect=load(d/"defect-ledger.json"); graph=load(DOCS/"DEPENDENCY_GRAPH.json"); own=load(DOCS/"PATH_OWNERSHIP.json")
    row=graph.get("tasks",{}).get(tid,{})
    if c.get("dependencies")!=row.get("dependencies"): errs.append("dependency mismatch")
    if set(c.get("required_critics",[]))!=set(row.get("critics",[])): errs.append("base critic mismatch")
    ...
    if c.get("resource_actor_contract_required") is not True or p.get("resource_actor_contract_required") is not True: errs.append("resource/actor contract not mandatory")
    if c.get("current_registry_hashes_required_at_activation") is not True or p.get("current_registry_hashes_required_at_activation") is not True: errs.append("registry hash rebinding not mandatory")
    if p.get("conditional_c5_on_animation_delta") is not True: errs.append("conditional C5 missing")
    if not c.get("conditional_critic_rules"): errs.append("conditional critic rule missing")
    ...
    for cand in paths:
        ...
        for owner in own.get("active_owners",[]):
            for foreign in own.get("aliases",{}).get(owner.get("paths_alias"),[]):
                if overlap(cand,foreign): errs.append(f"active collision {tid}:{cand} vs {owner.get('task_id')}:{foreign}")
    if defect.get("unresolved_preparation_defects"): errs.append("unresolved preparation defects")
    return {"task_id":tid,"graph_status":row.get("status"),"paths":len(paths),"resource_contract":True,"conditional_c5":True,"passed":not errs,"errors":errs}
def cross():
...
def main():
    ...
    if a.all:
        ...
        rs=[prep(t) for t in TASKS]; collisions=cross(); out={"wave":"T44-T52","results":rs,"cross_task_path_collisions":collisions,"passed":all(x["passed"] for x in rs) and not collisions}
    else:
