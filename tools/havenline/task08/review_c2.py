...
def main():
    ...
    spec,checks=resolve_critic('T08','C2',execution,matrix);dimensions=spec['dimensions']
    ...
    for item in m['files']:
        if shared.digest(cache/item['filename'])!=item['sha256']: raise SystemExit('critic runtime hash mismatch '+item['filename'])
    ...
    proc=subprocess.Popen([str(server),'-m',str(cache/m['model_file']),'--mmproj',str(cache/m['projector_file']),'--host','127.0.0.1','--port','8080','-c','12288','-t','4','-tb','4','-ngl','0','--no-mmproj-offload','--parallel','1','--jinja','--image-min-tokens','768','--image-max-tokens','2048'],stdout=log,stderr=subprocess.STDOUT,env=env)
    rows=[];fatal=None
    try:
        for _ in range(180):
        ...
        schema=shared.response_schema('T08',dimensions)
        prompt=f"""You are the independent {matrix['critics']['C2']['name']} for Havenline T08 candidate {candidate}. Review only the supplied exact-source rendered evidence. Mandatory checks: {json.dumps(checks)}. Inspect the authored transfer ribbons, directional sigils, destination bursts, resource-piece contact, clipping/seams, intentional gaps, visual attachment to the transfer path, and cross-view consistency. Primitive-looking or placeholder presentation is a concrete defect even if custom geometry is technically used. Do not invent defects and do not excuse visible ones. Every mandatory dimension is 0-10. PASS requires every unrounded score strictly above 9.0, zero actionable defects, complete coverage and at least medium confidence. Native 4K frames are review evidence, not physical-device certification. Return JSON only."""
        for group in manifest['groups']:
            board=shared.evidence_board(group,out);text=shared.evidence_text(group)
            review=shared.request_local(board,text,prompt+f"\nEvidence group: {group['id']}. Judge every mandatory C2 dimension.",schema,out,group['id'])
    ...
    rank={'low':0,'medium':1,'high':2};confidence=min((r.get('review',{}).get('confidence','low') for r in rows),key=lambda x:rank.get(x,0),default='low')
    raw={'critic_id':'C2','task_id':'T08','candidate':candidate,'groups':rows,'fatal_error':fatal}
    ...
    record={'task_id':'T08','critic_id':'C2','provider':m['publisher'],'model':m['base_model'],'model_revision_expected':runtime['model_revision'],'model_revision_actual':m['revision'],'runtime_release':m.get('runtime_release'),'request_or_run_id':os.environ.get('GITHUB_RUN_ID','local')+'/'+os.environ.get('GITHUB_JOB','C2'),'candidate_hash':candidate,'input_manifest_hash':shared.digest(manifest_path),'raw_output_path':str(raw_path.relative_to(ROOT)),'raw_output_hash':shared.digest(raw_path),'scores':dim_scores,'defects':defects,'coverage_complete':fatal is None and len(rows)==len(manifest['groups']) and all(r.get('review',{}).get('coverage_complete') is True for r in rows),'confidence':confidence,'independent_runtime':True,'groups':rows,'fatal_error':fatal,'passed':passed}
