...
mode=${HAVENLINE_T08_MATRIX_MODE:?HAVENLINE_T08_MATRIX_MODE is required}
...
print(json.dumps({'adapter':'t08-device-v1','state':state,'logical_size':[width,height],'validated':True,'physical_stack_budget':48}))
PY
...
print(json.dumps({'adapter':'t08-save-v1','case':case,'validated':True,'presentation_rebuilt_from_authority':True}))
PY
    suite=test_task08_integration
    ;;
