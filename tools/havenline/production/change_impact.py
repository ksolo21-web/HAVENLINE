...
from runtime_dependency_learning import learned_impact
    ...
    for path in files:
        ...
        for rule in matrix["impact_rules"]:
            if any_match(path,rule["patterns"]):
                ...
                if not rule.get("governance_only",False):governance_only=False
        ...
        elif matched and not all(r.get("governance_only",False) for r in matched):governance_only=False
    if unknown_production:
        for group in matrix["approved_task_suites"].values():suites.update(group)
        impacted.update(matrix["approved_task_suites"].keys())
    learned=learned_impact(files)
    impacted.update(learned["impacted_tasks"]);suites.update(learned["required_suites"])
    return {
      "changed_files":files,
      ...
      "runtime_observed_dependency_impact":learned,
      "runtime_learning_policy":"ADDITIVE_ONLY; static mandatory coverage is never removed automatically"
    }

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--base");ap.add_argument("--head",default="HEAD");ap.add_argument("files",nargs="*");a=ap.parse_args();files=a.files or changed_files(a.base,a.head);print(json.dumps(calculate(files),indent=2))
