...
assert metrics['t03_incremental_draw_batches']==2
assert metrics['collision_panels']==26
assert metrics['fence_visual_instances']==38
assert metrics['new_T03_GLBS']==0 and metrics['new_T03_textures']==0
...
prompt='''You are the independent Havenline C6 Performance Critic reviewing ONLY the incremental performance risk of Task T03 fences, gates and packed work lanes. This is an early subsystem budget gate, not final phone/tablet certification. T68/T69 separately require sustained physical native-4K/60 and thermals, so do not penalize T03 merely because those later hardware tests are not performed yet. Judge whether T03 consumes an unsustainable share of the future full-game budget from the supplied exact-source metrics. Use the appended JSON as the sole numeric authority: T03 adds two batched visual draw batches, 26 necessary collision sections and 38 fence visual instances; it reuses existing GLBs/material infrastructure and adds no texture, GLB, animation or NPC content. The scene draw-call and primitive maxima are whole-scene capture values, not T03 deltas, so they cannot support a T03-specific topology defect without a measured T03 primitive delta. Score draw_call_batching, geometry_increment, shader_increment, physics_increment, animation_npc_increment, memory_storage_increment, thermal_risk_proxy. Every score <=9.0 must cite an actionable T03-specific defect supported by an incremental metric. A >9 score means the incremental T03 load is comfortably bounded for this intermediate task. Return JSON only.'''
...
try:
 ...
 body={'model':'T03-C6-performance','messages':[{'role':'user','content':prompt}],'max_tokens':800,'temperature':.2,'top_p':.9,'seed':20260919,'repeat_penalty':1.12,'chat_template_kwargs':{'enable_thinking':False},'response_format':{'type':'json_object','schema':schema},'cache_prompt':False}
