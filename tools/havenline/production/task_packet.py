...
"""
    ...
    if task_id.startswith("T") and task_id[1:].isdigit() and int(task_id[1:]) >= 10:
        ...
        if int(task_id[1:]) >= 11:
            ...
            text+=f"- Claim ASSIGNED only through `python3 tools/havenline/production/v32_assignment_claim.py {task_id} --owner <OWNER> --branch <BRANCH> --base <EXACT_RECONCILED_INTEGRATION_HEAD> --owned-alias <ALIAS> --builder-head <EXACT_BUILDER_HEAD> --integration-head <EXACT_CURRENT_INTEGRATION_HEAD>`; it verifies the actual remote branch tips and records the assignment binding.\n"
