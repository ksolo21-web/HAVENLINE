...
"""
    ...
    if task_id.startswith("T") and task_id[1:].isdigit() and int(task_id[1:]) >= 10:
        ...
        if int(task_id[1:]) >= 11:
            ...
            text+="- The shared control plane owns exact candidate/integration binding, graduation, remote branch budget, gate-fingerprint decisions, synthetic-merge/contract checks, checkpoint resume validation, critic-package preflight, specialist fan-out, and C0 routing. C1/C2 and deterministic C6/C9 remain in their required task-specific lanes.\n"
