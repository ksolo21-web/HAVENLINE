...
class V32Tests(unittest.TestCase):
 ...
 def test_branch_and_performance_ledgers_validate(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   self.assertFalse(branch_budget.validate(inventory=str(p))['passed'])
   p.write_text(json.dumps(['havenline/T11-camp-construction','havenline/T12-progression-architecture']))
   seeded=branch_budget.validate(inventory=str(p))
   self.assertTrue(seeded['passed'],seeded['errors'])
   self.assertEqual('havenline/T12-progression-architecture',seeded['details']['authorized_preactivation_branches'].get('T12'))
