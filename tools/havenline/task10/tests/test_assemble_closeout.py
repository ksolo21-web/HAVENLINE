...
class CloseoutTests(unittest.TestCase):
    ...
    def test_full_assembler_proposed_flow_with_disposable_diagnostic_boundary_patch(self):
        ...
        with tempfile.TemporaryDirectory(prefix='t10-synthetic-no-remote-') as temporary:
            ...
            defects=[dict(id=row['id'],classification=row['classification'],status='VERIFIED_CLOSED',affected_object='DIAGNOSTIC ONLY',
                          causal_files=['diagnostic-fixture'],command_or_gate='G1',reviewer_or_owner='synthetic-test-reviewer',
                          proof_candidate=candidate,unresolved=False,verification_proofs=[proofs['G1']]) for row in c0['blockers']]
