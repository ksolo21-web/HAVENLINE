...
PROMPT+='\nOutput discipline only: keep each observation and defect to one sentence of at most 24 words. Never repeat a phrase or sentence.'
...
def query(image,prompt,schema,name,max_tokens=650):
 ...
 # Dense T03 boards can spend more than twenty minutes in CPU prompt
 # evaluation before generation begins. Preserve the exact model, pixels,
 # rubric and seed, but allow the request to finish instead of manufacturing an
 # incomplete shard at the old 1,200-second transport ceiling.
 with urllib.request.urlopen(req,timeout=1800) as resp:result=json.load(resp)
