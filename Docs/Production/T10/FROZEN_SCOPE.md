...
- **T10-R13 — Dependency truth.** Production implementation may not enter `ASSIGNED`/`BUILDING_ISOLATED` until T05, T08 and T09 are all `APPROVED`. PREPARED governance, tests, validators, workflow and activation tooling may be completed while T09 is active.
...
The T10 isolated builder will own only the following disjoint paths once activation is legal:
...
- Exact post-T09 integration base, candidate hash and authorized changed-file manifest.
...
- Resource conservation proof against T08; T09-acquired resource compatibility after T09 is approved and integrated.
- Neutral fixture sequences showing before, ready/preview, committing and after from gameplay, overhead, side/three-quarter and detail views; adaptive phone/tablet/foldable coverage and native 3840x2160 scale-1 evidence where applicable.
...
- Full impacted T01-T09 regression after integration.
