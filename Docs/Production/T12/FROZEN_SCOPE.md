...
- **T12-R01 — Stable Level 1–100 identity.** Shipping progression defines exactly one stable record for each integer level 1 through 100, with deterministic IDs, ordering and explicit prerequisite/milestone metadata. Duplicate, missing, out-of-range or cyclic level topology fails closed.
...
- **T12-R09 — Recovery-compatible identifiers.** Level, milestone, region-band, unlock and event IDs are stable so T14 can persist/recover them without redefining T12 semantics. T12 does not implement the global save/versioning layer.
...
- **T12-R15 — Dependency truth.** T12 production implementation may not enter `ASSIGNED`/`BUILDING_ISOLATED` until T07, T08, T10 and T11 are all `APPROVED`, with T10/T11 integrated on the authoritative integration branch. Governance packet/tooling preparation may complete while T09/T10/T11 runtime work is unfinished.
