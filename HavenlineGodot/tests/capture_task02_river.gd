...
func bank_point(x:float,north:bool,extra:=0.85)->Vector2:
	var q:Dictionary=River.query(River.center_at_x(x));var side:=1.0 if north else -1.0
...
func snap_named(name:String):
	await snap(name);records[-1]["river_layout_version"]=River.LAYOUT_VERSION
func run():
	...
	if native:
		# Eleven source-bound 4K frames: full authored terrain, current map, both
		# entries, four representative bends, two bank contacts and camp relation.
		...
		for x_value in [-10.0,-3.0,4.0,11.0]:
			var x:float=float(x_value);focus_at(River.center_at_x(x),Vector3(0,14,17),10.5);await snap_named("native-bend-%s"%str(int(x)).replace("-","m"))
		...
		# Full authored terrain proves the same river already exists in future land.
		...
		var bend_x:Array=[-24.0,-17.0,-10.0,-3.0,4.0,11.0,19.0,25.0]
		for i in range(bend_x.size()):
			focus_at(River.center_at_x(float(bend_x[i])),Vector3(0,14,17),10.5);await snap_named("river-bend-%02d"%i)
		for item_value in [[-10.0,true,"north-west"],[0.0,true,"north-centre"],[10.0,true,"north-east"],[-10.0,false,"south-west"],[0.0,false,"south-centre"],[10.0,false,"south-east"]]:
			var item:Array=item_value;normal_at(bank_point(float(item[0]),bool(item[1])));await snap_named("river-gameplay-"+str(item[2]))
		...
		for i in range(River.CROSSING_X.size()):
			var x:float=float(River.CROSSING_X[i]);focus_at(River.center_at_x(x),Vector3(0,19,.01),12.0);await snap_named("crossing-reserve-%02d"%i)
		...
		# Ordered simulation-time samples verify visible flow continuity, not FPS.
		focus_at(River.center_at_x(0),Vector3(0,10,12),8.0)
		for i in range(6):game.sim.climate.seconds+=.45;game.outpost_view.sync(game.sim,.45,false);await snap_named("river-flow-%02d"%i)
		# Identical camera over environmental states catches bank/material regressions.
		var held:=0
		for animation in game.world.find_children("*","AnimationPlayer",true,false):animation.pause();held+=1
		for setting_value in [["day",0.0],["dusk",400.0],["night",930.0],["dawn",990.0],["blizzard-night",630.0],["day-return",0.0]]:
			var setting:Array=setting_value;game.sim.climate.seconds=float(setting[1]);game.outpost_view.sync(game.sim,.1,false);await snap_named("river-condition-"+str(setting[0]))
			records[-1]["held_unrelated_animation_players"]=held;records[-1]["hour"]=game.sim.climate.hour();records[-1]["weather"]=game.sim.climate.weather()
