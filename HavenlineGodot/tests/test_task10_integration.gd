...
func run() -> void:
	...
	check("all four resource shortfalls and additional prerequisite remain visible", beacon.text.begins_with("Foundation blocked — deliver") and beacon.text.contains("1 wood") and beacon.text.contains("2 stone") and beacon.text.contains("3 metal") and beacon.text.contains("4 fuel") and beacon.text.contains("harvesting access") and beacon.text.contains("\ue002") and beacon.text.contains("\ue003"))
	multi_block.shortfalls = {}
	multi_block.errors = ["missing_prerequisite:harvesting_online"]
	view.show_blocked(multi_block)
	check("prerequisite-only block does not invent missing resources", beacon.text.begins_with("BLOCKED — Foundation\nNeeds harvesting access") and not beacon.text.contains("deliver for"))
	view.set_ready()
	...
	check("preview world response shows ring plus translucent preview volume", view.show_preview(visual_preview) and ring.visible and ghost.visible and beacon.visible and beacon.text.begins_with("PREVIEW — stay near") and beacon.text.contains("Cost from delivered stock:") and ghost.material_override.transparency == BaseMaterial3D.TRANSPARENCY_ALPHA)
	...
	check("pending resources have exact named source-to-target cues without premature payment", beacon.text.begins_with("APPLYING → Foundation") and beacon.text.contains("Delivered resources:") and beacon.text.contains("8 wood") and beacon.text.contains("4 stone") and not beacon.text.contains("Spent:") and view._ring_material.emission_enabled)
	...
	check("completion puts next-stage shortfalls first and preserves prerequisite", beacon.text.begins_with("NEXT Reinforced — deliver") and beacon.text.contains("12 wood") and beacon.text.contains("8 stone") and beacon.text.contains("2 metal") and beacon.text.contains("harvesting access"))
