...
func _settle(p:Vector2,margin:float)->Vector2:
	var q:=p
	for _i in range(4):q=Surface.land_position(q,margin)
	return q
func run():
	...
	for zi in range(-162,163,2):
		for xi in range(-142,143,2):
			...
			var q:=_settle(p,River.DEFAULT_DRY_MARGIN);var r:=_settle(q,River.DEFAULT_DRY_MARGIN);samples+=1
	...
	check("Repeated river projection is stable",stable)
	...
	for north in [true,false]:
		...
		for xi in range(-12,13,2):
			...
			for _frame in range(900):
				...
				movement_safe=movement_safe and River.shore_distance(sim.position)>=River.DEFAULT_DRY_MARGIN-.002
			check(("North" if north else "South")+" bank route reaches x="+str(xi),reached)
	check("Real movement along both banks never enters river water",movement_safe)
	...
	for x_value in [-14.0,-10.0,-6.0,-3.0,0.0,1.5,5.0,10.0,14.0]:
		...
		for side_value in [-1.0,1.0]:
			...
			var rq:Dictionary=River.query(restored.position);legacy_ok=legacy_ok and float(rq.side)==side and float(rq.shore_distance)>=River.DEFAULT_DRY_MARGIN-.002
	check("Legacy wet saves across both banks preserve state and river side",legacy_ok)
	...
	print(JSON.stringify({"suite":"T02_river_surface_stress","checks":checks,"failures":failures,"passed":failures.is_empty(),"projection_samples":samples,"legacy_save_cases":legacy_cases,"lane_samples":lane_points.size(),"physical_4k60_verified":false,"independent_critic":false}))
