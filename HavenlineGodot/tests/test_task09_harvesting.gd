...
func run() -> void:
	...
	check("distributed shoulder-chain extension is explicit and bounded", contract.maximum_distributed_arm_extension_meters == 0.34)
	check("contact targets are authored on rendered source surfaces", contract.source_contact_policy == "authored_visible_surface_anchor")
	check("head clearance participates in the contact gate", contract.minimum_head_clearance_meters == 0.18)
	...
	source_visual.position = Vector3(2.0,1.0,3.0)
