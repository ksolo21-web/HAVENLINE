...
func run() -> void:
	...
	check("exact T06 impact progress opens contact", wood_state.contact_ready)
	...
	for resource_value in ["stone","metal","fuel"]:
		...
		check(resource + " uses exact tool and contact", state.tool == profile.tool and state.contact_marker == profile.contact_marker and state.contact_ready)
