...
func run():
	...
	check("Authored fence roots are deliberately sunk into terrain",is_equal_approx(float(desc.fence_root_sink),.34))
	check("Fence joins open leaves tall portal posts and rutted terrain preserve strong threshold contact",is_equal_approx(float(desc.visual_join_overlap),.10) and is_equal_approx(float(desc.visual_corner_join_overlap),.28) and is_equal_approx(float(desc.gate_hinge_overlap),.12) and is_equal_approx(float(desc.gate_leaf_root_sink),.50) and is_equal_approx(float(desc.gate_leaf_hinge_sink),.34) and int(desc.terrain_seat_samples)==7 and desc.terrain_crown_applied_to_gate_leaves_only and is_equal_approx(float(desc.gate_post_root_sink),.30) and is_equal_approx(float(desc.main_gate_post_scale),1.15) and is_equal_approx(float(desc.main_gate_post_height_scale),1.55) and is_equal_approx(float(desc.river_gate_post_scale),1.24) and is_equal_approx(float(desc.river_gate_post_height_scale),1.85) and is_equal_approx(float(desc.work_gate_post_scale),1.18) and is_equal_approx(float(desc.work_gate_post_height_scale),1.65) and is_equal_approx(float(desc.lane_compression_depth),.075) and is_equal_approx(float(desc.lane_rut_depth),.055) and is_equal_approx(float(desc.lane_shoulder_height),.075))
