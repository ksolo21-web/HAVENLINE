...
func run():
	...
	check("Fence joins open leaves tall portal posts and rutted terrain preserve strong threshold contact",is_equal_approx(float(desc.visual_join_overlap),.18) and is_equal_approx(float(desc.visual_corner_join_overlap),.28) and is_equal_approx(float(desc.gate_hinge_overlap),.12) and is_equal_approx(float(desc.gate_leaf_root_sink),.56) and is_equal_approx(float(desc.gate_leaf_hinge_sink),.40) and int(desc.terrain_seat_samples)==7 and desc.terrain_crown_applied_to_gate_leaves_only and is_equal_approx(float(desc.gate_post_root_sink),.40) and is_equal_approx(float(desc.main_gate_post_scale),1.00) and is_equal_approx(float(desc.main_gate_post_height_scale),1.05) and is_equal_approx(float(desc.river_gate_post_scale),1.05) and is_equal_approx(float(desc.river_gate_post_height_scale),1.08) and is_equal_approx(float(desc.work_gate_post_scale),1.00) and is_equal_approx(float(desc.work_gate_post_height_scale),1.00) and is_equal_approx(float(desc.lane_compression_depth),.075) and is_equal_approx(float(desc.lane_rut_depth),.055) and is_equal_approx(float(desc.lane_shoulder_height),.075))
