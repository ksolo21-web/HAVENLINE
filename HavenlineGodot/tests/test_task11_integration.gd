...
func run() -> void:
	...
	check("T11 preview explains automatic delivered-stock application", view.descriptor().auto_build_guidance.contains("delivered stock applies automatically") and view.descriptor().delivered_stock_flow_visible)
	...
	check("T11 committing presentation requires no extra player input", view.descriptor().auto_build_guidance.contains("no input needed") and view.descriptor().delivered_stock_flow_visible)
	...
	check("accepted receipt stops T11 resource flow", not view.descriptor().delivered_stock_flow_visible and view.descriptor().auto_build_guidance.contains("BUILD COMPLETE"))
	...
	check("reinforced preview retains automatic build guidance", view.descriptor().auto_build_guidance.contains("AUTO-BUILD READY") and view.descriptor().delivered_stock_flow_visible)
