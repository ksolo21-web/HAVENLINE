...
func run():
	...
	var report={"task":"T03-boundary-v1","renderer":RenderingServer.get_current_rendering_method(),"device":RenderingServer.get_video_adapter_name(),"captures":records,"forest":game.reference_forest_evidence,"river":game.outpost_view.evidence(game.sim),"boundary":game.camp_boundary_view.descriptor,"disclosed_qa_cameras":true,"conditions_hold_unrelated_animations_only":true,"native_frames_are_not_physical_fps_evidence":true,"task_approved":false}
