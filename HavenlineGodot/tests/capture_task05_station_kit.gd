...
func family_positions(count: int) -> Array[Vector3]:
	...
	for index in range(count):
		...
		result.append(Vector3((float(column) - float(columns - 1) * 0.5) * 3.9, 0.0, (float(row) - float(rows - 1) * 0.5) * 3.5))
	return result

func capture_family(family_id: String, asset_ids: Array[String], view: String, full_height: float) -> void:
	camp_stage()
	reset_kit()
	var positions := family_positions(asset_ids.size())
	for index in range(asset_ids.size()):
		var asset := kit.instantiate_asset(asset_ids[index])
		asset.position = positions[index]
		kit.placed.append(asset)
	scale_actor.position = Vector3(float(mini(4, asset_ids.size())) * 2.15, 0.0, 2.8)
	scale_actor.visible = true
	set_camera(Vector3(0.0, 0.0, 0.0), view, full_height * (0.88 if view == "detail" else 1.0))
	set_condition("day")
	await snap("family-" + family_id + "-" + view, family_id, "day", view)

func capture_family_gallery() -> void:
	var families := [
		{"id":"hearth", "assets":["hearth_vessel"], "height":4.6},
		{"id":"counters", "assets":["service_counter", "processing_counter"], "height":6.1},
		{"id":"pads", "assets":["pad_build", "pad_upgrade", "pad_input", "pad_output", "pad_stock", "pad_payment"], "height":9.0},
		{"id":"fishing", "assets":["fishing_rack", "intake_machine", "fish_crate"], "height":7.3},
		{"id":"processing", "assets":["intake_machine", "conveyor_straight", "cooker_processor", "pad_output"], "height":8.4},
		{"id":"defense", "assets":["defense_platform"], "height":5.0},
		{"id":"resources", "assets":["wood_stack", "stone_stack", "metal_stack", "fuel_canister", "fish_crate", "cooked_food_stack", "money_stack", "cargo_crate"], "height":9.2}
	]
	var views := ["front", "rear", "left", "right", "three-quarter", "detail"]
	for family: Dictionary in families:
		var assets: Array[String] = []
		assets.assign(family.assets)
		for view: String in views:
			await capture_family(str(family.id), assets, view, float(family.height))
...
func run() -> void:
	...
	var report := {
		...
		"capture_kind": "isolated-family-coverage" if family_gallery else "isolated-component-quick-look",
		...
		"captures": records,
		"required_family_views": ["front", "rear", "left", "right", "three-quarter", "detail"] if family_gallery else [],
		"family_count": 7 if family_gallery else 0,
