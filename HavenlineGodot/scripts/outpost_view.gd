...
# Kept as `lake`/`lake_material` aliases so existing capture tooling does not
# break while Task 2 is re-reviewed. The runtime node is a river.
...
func configure(env: Environment, sun: DirectionalLight3D, furnace: Node3D, light: OmniLight3D, sim):
	...
	var rng:=RandomNumberGenerator.new();rng.seed=754219
	...
	snow_material=ShaderMaterial.new();snow_material.shader=load("res://shaders/outpost_snowfall.gdshader")
	snow.material_override=snow_material
	add_child(snow)
	sync(sim,0.0,true)

func sync(sim, dt: float, paused: bool):
	var weather: Dictionary=sim.climate.weather();var daylight: float=sim.climate.daylight()
	...
	environment.fog_depth_begin=27.0;environment.fog_depth_end=67.0;environment.fog_depth_curve=1.35
...
func evidence(sim) -> Dictionary:
	var river:=Surface.river_evidence()
	return {"clock":sim.climate.snapshot(),"hour":sim.climate.hour(),"weather":sim.climate.weather(),
		...
		"surface_triangles":terrain.mesh.get_faces().size()/3,"terrain_revision":"T02-river-v1-mapspan",
		"river":river,"river_layout_version":River.LAYOUT_VERSION,"water_triangles":lake.mesh.get_faces().size()/3,
		"water_y":Surface.WATER_Y,"material_debanding_enabled":material_debanding_enabled,
		"west_east_mapspan":true,"shared_actor_surface":true,"native_4k60_certified":false}
